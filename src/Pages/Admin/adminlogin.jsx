...
const AdminLogin = () => {
  ...
  // UseEffect to check if "Remember Me" is enabled and pre-fill credentials
  useEffect(() => {
  ...
  const handleLogin = async () => {
    ...
    if (username && password && isCaptchaVerified) {
      ...
      if (!value) {
        error.captcha.textContent = "Please check the box to proceed";
        return;
      }
      setLoading(true);
      try {
        ...
        if (response.data.http_status_code === 200) {
          ...
          Cookies.set("is_vendor", response.data.is_vendor);
          Cookies.set("status", response.data.http_status_code);
          Cookies.set("subroles", JSON.stringify(subRole));
        ...
        setError((prev) => ({
          ...prev,
          credentials: "Please enter correct username and password",
        }));
      } finally {
        setLoading(false);
      }
            ...
            <div className="flex flex-col items-center justify-center mt-10 h-full">
              ...
              <div className="scale-[0.75] flex flex-col ml-[-80px] w-full">
                <ReCAPTCHA
                  ...
                  id="captcha"
                />
                <span className="text-red-500 font-semibold text-[20px] mt-1">
                  {error.captcha}
                </span>
              </div>
